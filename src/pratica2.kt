class pratica2 {
    ...
    fun editarLivro(repositorio: MutableList<Livro>) {
        val livro = buscarNome(repositorio)
        if (livro != null) {
            println("Digite o novo título do livro (ou pressione Enter para manter o título atual):")
            val novoTitulo = readLine() ?: livro.titulo

            println("Digite o novo preço do livro (ou pressione Enter para manter o preço atual):")
            val novoPrecoInput = readLine()
            val novoPreco = novoPrecoInput?.toFloatOrNull() ?: livro.preco

            val novoLivro = Livro(novoTitulo, novoPreco)
            val index = repositorio.indexOf(livro)
            repositorio[index] = novoLivro
